...
#include <stdlib.h> //stdlib.h contains malloc
...
struct list* list_create() {
  struct list *ptr = malloc(sizeof(*ptr));
  ptr->head = NULL;
  return ptr;
}
...
void list_insert(struct list* list, void* val) {
  //cant initalize non-pointes?
  //does the pointer create the object?
  //writing struct link node creates it on the stack, in order to make it on the heap it needs to be a pointer to an object
  struct link *node = malloc(sizeof(*node));
  struct link *temp = malloc(sizeof(*temp));
  temp->next = list->head;
  node->val = val;
  list->head = node;
  node->next = temp->next;
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
  struct link *one = list->head;
  struct link *two = list->head;
  struct link *three = list->head;
  three=three->next;
  if(cmp(val, two->val)==0){
    free(two);
    list->head = three;
  }
  else{
