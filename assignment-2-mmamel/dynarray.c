...
void edge_case(struct dynarray* array, void* val){
  void** temp = malloc(array->capacity * sizeof(void*) * 2);
  for(int i = array->logical; i<array->capacity; i++){
    temp[i - array->logical] = array->data[i];
  }
  for(int i = 0;i<array->logical; i++){
    temp[i+array->logical] = array->data[i];
  }
  temp[array->size] = val;
  array->size++;
...
void* dynarray_get(struct dynarray* da, int idx) {
  assert(da);
  assert(idx < da->size && idx >= 0);
// printf("this is index %d this is size %d", idx, da->size);
  return da->data[idx];
}
