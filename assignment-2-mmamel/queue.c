...
struct queue* queue_create() {
  ...
  queue_ptr->array = dynarray_create();
  return queue_ptr;
}
...
void queue_enqueue(struct queue* queue, void* val) {
  ...
  int capacity = get_capacity(queue->array);
  int logical = get_logical(queue->array);
  if(size < capacity){
    if(logical == 0){
      dynarray_insert(queue->array, val);
    }
    else{
      int index = size - logical;
      insert(queue->array,index, val);
      increment_size(queue->array);
    }
  }
  else{
    edge_case(queue->array, val);
  }
  //printf("this is logical %d and this is size %d", logical, dynarray_size);
  return;
}
...
void* queue_front(struct queue* queue) {
  int index = get_logical(queue->array);
  return get_value(queue->array, index);
}
...
void* queue_dequeue(struct queue* queue) {
  int logical = get_logical(queue->array);
  void* pop = get_value(queue->array, logical);
  increment_logical(queue->array);
  decrement_size(queue->array);
  return pop;
}
